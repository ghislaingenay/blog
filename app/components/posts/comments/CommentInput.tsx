...
export const CommentInput = ({ dict }: CommentInputProps) => {
  ...
  const commentBody: Omit<CommentAttrs, "message"> = {
    postId: queryParams.postId,
    email: user?.email || ("" as string),
    userPicture: user?.picture || (falbackPicture as string),
  ...
  async function action() {
    await createComment({ ...commentBody, message: deferredComment });
  }
  ...
  return (
    <form
      className="px-5 py-2 bg-slate-200 rounded-lg gap-4 flex justify-between"
      action="/api/hello"
    >
      <input type="hidden" name="postId" value={commentBody.postId} />
      <input type="hidden" name="email" value={commentBody.email} />
      <input type="hidden" name="userPicture" value={commentBody.userPicture} />
      <input
        type="hidden"
        name="language"
        value={commentBody.language as string}
      />
      <input type="hidden" name="name" value={commentBody.name as string} />
